...
function M.buffer(dont_focus)
  ...
  local function mark(original, hl)
    ...
    return {
      ...
      text = vim.schedule_wrap(function(content)
        local res = vim.api.nvim_buf_get_extmark_by_id(
          buf,
          ns,
          id,
          { details = true }
  ...
  local function get_mark_at_cursor()
    ...
    for _, m in ipairs(marks) do
      ...
      local after_start = row > pos.start.row or
        (row == pos.start.row and col >= pos.start.col)

      local before_stop = row < pos.stop.row or
        (row == pos.stop.row and col <= pos.stop.col)
  ...
  local function handle_cr()
    ...
    if not success then error(id) end
  ...
  if not dont_focus then focus() end
